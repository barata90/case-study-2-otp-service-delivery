...
The workflow covers KPI monitoring (OTP and delay metrics), segment diagnostics, anomaly detection, interpretable drivers of satisfaction, and leakage-aware severe-delay risk scoring, plus an interactive Plotly Dash dashboard.
...
Recommended GitHub layout (aligned with the current folder structure and typical recruiter expectations):

```
case-study-2-otp-service-delivery/
  README.md
  README_DASH.md
  requirements.txt
  app.py
...
> Note: `train.csv` and `test.csv` are intentionally not listed in the GitHub layout because datasets are often excluded from version control.  
> A lightweight approach is linking to the Kaggle dataset and keeping data locally.
...
- **Demo video (local path in this portfolio folder):**  
  `assets/Screen Recording 2026-01-18 at 14.52.04.mov`

- **Screenshots:**  
  Place key UI screenshots under `assets/screenshots/` (Overview, Service Delivery, Model Drivers, Severe Delay Risk).

> GitHub note: `.mov` files can exceed GitHub’s recommended file limits.  
> If the demo video is large, common options include (1) Git LFS, or (2) uploading to Drive/YouTube and adding a share link here.

---

## Publish to GitHub (minimal steps)
...
git init
git add .
git commit -m "Add Case Study 2: OTP & Service Delivery dashboard and notebook"
git branch -M main
...
- Keep `train.csv` and `test.csv` local and exclude them from Git history.
- Provide a short note in the repository pointing to the Kaggle dataset page.
- If the notebook requires the files, the default expectation is: `train.csv` and `test.csv` placed next to `app.py` / notebook.
